...
CAssign[lhs_:"",expr_?(!OptionQ[#]&),opts___?OptionQ]:=
  Module[{optvals},
    optvals /; 
      And[
        (optvals = OptionTest[expr,GetShape[lhs],CAssign,System`Utilities`FilterOptions[CAssign,opts]])=!=$Failed,
        optvals = CMain[lhs,expr,optvals,{System`Utilities`FilterOptions[Experimental`OptimizeExpression,opts]}];
        True
      ]
  ];
    ...
    optvals /;
      And[
        (optvals = OptionTest[expr,GetShape[lhs],FortranAssign,
          System`Utilities`FilterOptions[FortranAssign,opts]])=!=$Failed,
        optvals = FMain[lhs,expr,optvals,{System`Utilities`FilterOptions[Experimental`OptimizeExpression,opts]}];
        True
      ]
  ];
