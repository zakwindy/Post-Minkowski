...
#--------------------------------------------------
typestring = "newton"
dfa_in.to_csv(typestring + '_a_in_' + bodies + '_' + conditions + '_' + str(half) + '.csv', index=False)
dfa_out.to_csv(typestring +  '_a_out_' + bodies + '_' + conditions + '_' + str(half) + '.csv', index=False)
dfe_in.to_csv(typestring +  '_e_in_' + bodies + '_' + conditions + '_' + str(half) + '.csv', index=False)
dfe_out.to_csv(typestring + '_e_out_' + bodies + '_' + conditions + '_' + str(half) + '.csv', index=False)
dfi.to_csv(typestring + '_i_' + bodies + '_' + conditions + '_' + str(half) + '.csv', index=False)

#--------------------------------------------------
#   transform data to physical units
#--------------------------------------------------
t = data['timestep']*T/(3600*24*365)    #put time data in units of years
a_in_AU = a_in * L / KM_CGS             #put semi-major axes in AU
a_out_AU = a_out * L / KM_CGS
...
plt.figure()
plt.plot(t, mutual_i_degree)
plt.xlabel('Time (years)')
plt.ylabel('Mutual inclination in Degrees')
plt.savefig('i_mut_' + bodies + '_' + conditions + '.png')

plt.figure()
plt.plot(t, a_in_AU)
plt.xlabel('Time (years)')
plt.ylabel('Inner Orbit Semimajor Axis (KM)')
...
plt.plot(t, a_out_AU)
plt.xlabel('Time (years)')
plt.ylabel('Outer Orbit Semimajor Axis (KM)')
#plt.gca().yaxis.set_major_formatter(mtick.FormatStrFormatter('%.6f'))
